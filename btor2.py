...
"""
    Convert a model, provided by z3 solver, into output
    in the witness format (Btor2)
"""

def model2witness( model, steps, inputs ) -> None:
    if model is None:
        print( "unsat" )
        return
    print( "b0" )
    print( "#0 ")  #TODO: Check if all states are initialized
    for step in range( steps ):
        print( "@%d" %step )
        for idx, val in enumerate(inputs):
            if step != 0:
                val = freshInput( val.sort(), val, step )
            v_inter = model.get_interp( val )
            truth_value = 0 if v_inter is None or v_inter is False else 1
            val = val if step != 0 else freshInput( val.sort(), val, step )
            print( idx, truth_value, val ) 
    print( "." )
...
def bmc2model(ts: Ts, k: int) -> (z3.Solver.model, int ):
    ...
    while count < k:
        ...
        nfvs = [freshInput( x.sort(), x, count ) for x in fvs]
...
def main() -> None:
    ...
    #print( model )
    model2witness( model[0], model[1], ts.inputs )
